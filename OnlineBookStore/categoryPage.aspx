...
<head runat="server">
    ...
    <style>
        ...
        .search-input { width: 100%; max-width: 400px; padding: 8px 12px; border: 1px solid #ccc; border-radius: 20px 0 0 20px; border-right: none; font-size: 1rem; font-family: Arial, sans-serif; }
        .search-button { padding: 8px 12px; border: 1px solid #ccc; border-radius: 0 20px 20px 0; background-color: #f0f0f0; cursor: pointer; font-size: 0.9rem; }
        .search-button:hover { background-color: #e0e0e0; }
        .header-icons { display: flex; gap: 15px; font-size: 0.95rem; }

        /* [เพิ่ม] CSS ตะกร้า */
        .cart-count { display: inline; color: white; background-color: #d90000; border-radius: 50%; padding: 2px 6px; font-size: 0.8rem; }
        ...
        /* [แก้ไข] CSS การ์ด */
        .book-card { background-color: #fff; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 5px rgba(0,0,0,0.05); display: flex; flex-direction: column; transition: transform 0.3s, box-shadow 0.3s; }
        ...
        .book-author { font-size: 0.85rem; color: #555; margin: 6px 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        ...
        .no-items-panel { border: 1px dashed #e2e8f0; border-radius: 0.5rem; padding: 1.5rem; text-align: center; color: #6b7280; background-color: #fafafa; }
        ...
        /* --- [เพิ่ม] Modal Styles --- */
        .modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.6); display: flex; align-items: center; justify-content: center; z-index: 1000; opacity: 0; visibility: hidden; transition: opacity 0.3s, visibility 0s 0.3s; }
        .modal-overlay.show { opacity: 1; visibility: visible; transition: opacity 0.3s; }
        .modal-content { background: #fff; padding: 25px; border-radius: 8px; box-shadow: 0 5px 15px rgba(0,0,0,0.3); z-index: 1001; width: 90%; max-width: 600px; transform: translateY(-50px); transition: transform 0.3s ease-out; position: relative; }
        .modal-overlay.show .modal-content { transform: translateY(0); }
        .modal-close { position: absolute; top: 10px; right: 15px; font-size: 28px; font-weight: bold; color: #aaa; cursor: pointer; line-height: 1; }
        .modal-close:hover { color: #333; }
        .modal-body { display: flex; flex-direction: column; gap: 20px; margin-top: 15px; }
        @media (min-width: 500px) { .modal-body { flex-direction: row; } }
        .modal-book-cover { width: 100%; max-width: 200px; height: auto; aspect-ratio: 2 / 3; object-fit: cover; border-radius: 4px; border: 1px solid #eee; flex-shrink: 0; margin: 0 auto; }
        @media (min-width: 500px) { .modal-book-cover { width: 200px; height: 300px; margin: 0; } }
        .modal-details { flex-grow: 1; }
        .modal-details h3 { margin-top: 0; font-size: 1.6rem; }
        .modal-details p { margin: 6px 0; }
        .modal-book-price { font-size: 1.4rem; font-weight: bold; color: #d90000; margin-top: 10px; }
        .modal-book-meta { font-size: 0.9rem; color: #555; margin: 10px 0; border-top: 1px solid #f0f0f0; padding-top: 10px; }
        .modal-book-meta span { display: block; margin-bottom: 4px; }
        .modal-review-summary { margin-top: 10px; padding-top: 10px; border-top: 1px solid #f0f0f0; }
        ...
        .modal-btn-add { background-color: #d90000; color: white; border: none; padding: 10px 20px; font-size: 1rem; font-weight: bold; border-radius: 5px; cursor: pointer; transition: background-color 0.2s; }
        .modal-btn-add:hover { background-color: #b00000; }
        .modal-btn-add:disabled { background-color: #ccc; cursor: not-allowed; }
        .modal-btn-cancel { background-color: #f0f0f0; color: #333; border: 1px solid #ccc; padding: 10px 20px; font-size: 1rem; border-radius: 5px; cursor: pointer; margin-right: 10px; }
        .modal-btn-cancel:hover { background-color: #e0e0e0; }
        .js-open-modal { cursor: pointer; }
        .js-open-modal:hover { }
        .book-title.js-open-modal:hover { text-decoration: underline; }
        /* --- [จบ] Modal Styles --- */
...
<body>
    ...
    <script type="text/javascript">
        // ... (JavaScript เหมือนเดิมจาก topSalePage) ...
        ...
        function openBookDetailModal(clickedElement) {
            ...
            if (reviewCount > 0 && avgRating > 0) {
                ...
                modalBookReviews.innerHTML = `<span class="stars">${stars}</span><span class="review-count">(${avgRating.toFixed(1)} จาก ${reviewCount} รีวิว)</span>`;
            } else {
                modalBookReviews.innerHTML = `<span class="stars no-rating">☆☆☆☆☆</span><span class="review-count">(ยังไม่มีรีวิว)</span>`;
            }
        ...
        document.addEventListener('DOMContentLoaded', function () {
            document.querySelectorAll('.js-open-modal').forEach(element => {
                element.addEventListener('click', function () { openBookDetailModal(this); });
            });
        });
